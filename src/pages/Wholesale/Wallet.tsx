...
export function WholesaleWallet() {
  ...
  const totalDeposits = transactions.filter(t => t.type === 'deposit' && t.status === 'completed').reduce((sum, t) => sum + Number(t.amount), 0);
  const totalSpent = transactions.filter(t => t.type === 'order_payment' && t.status === 'completed').reduce((sum, t) => sum + Number(t.amount), 0);

  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Wholesale Wallet</h1>
          <p className="text-slate-500 mt-1">Manage B2B funds and track transactions</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-slate-900 rounded-2xl p-6 sm:p-8 relative overflow-hidden shadow-lg border border-slate-800">
            <div className="relative z-10">
              ...
              <Link to="/wholesale/deposit" className="w-full inline-flex items-center justify-center px-4 py-3 bg-emerald-500 hover:bg-emerald-400 text-white font-medium rounded-xl transition-colors shadow-sm">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 space-y-4">
            <div className="flex justify-between items-center pb-4 border-b border-slate-100">
              ...
              <span className="font-bold text-slate-900">৳{totalDeposits.toFixed(2)}</span>
            </div>
