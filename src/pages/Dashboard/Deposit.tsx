  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-slate-500 mt-1">Add money to your wallet to place orders instantly</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">Send the desired amount to our official numbers below using the "Send Money" option.</p>
                  </div>
                </div>
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">Copy the TrxID from your payment confirmation SMS or app screen.</p>
                  </div>
                </div>
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">Fill out the form on the right with the exact details. Funds will be added upon verification.</p>
                  </div>
                </div>
              </div>

              <div className="pt-6 border-t border-slate-800 mt-6">
                <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Official Accounts</h3>
                <div className="space-y-3">
                  {paymentMethods.map(method => (
                    <div key={method.id} className="flex justify-between items-center bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
                      ...
                      <span className="font-mono text-emerald-400 font-medium">{method.account_identifier}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-5 flex items-start">
            ...
            <div>
              <h4 className="font-semibold text-emerald-900 mb-1">Secure Transactions</h4>
              <p className="text-emerald-800/80 text-sm">All deposits are manually verified by our team. Please allow up to 15-30 minutes for processing during business hours.</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sm:p-8">
          <h2 className="text-xl font-bold text-slate-900 mb-6">Submit Deposit Request</h2>
            ...
            <div>
              ...
              {selectedMethod && (
                <p className="text-xs text-slate-500 mt-2">
                  Min: {selectedMethod.min_amount ? `৳${selectedMethod.min_amount}` : 'None'} | 
                  Max: {selectedMethod.max_amount ? `৳${selectedMethod.max_amount}` : 'None'}
                </p>
              )}
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Sender {selectedMethod?.name} Number</label>
              <input
                type="text"
                required
