  ...
  const completeOrder = async () => {
    ...
    try {
      ...
      if (deliveryFile) {
        ...
        const { error: uploadError, data: uploadData } = await supabase.storage.from("avatars").upload(`deliveries/${fileName}`, deliveryFile);
        if (uploadError) throw uploadError;
        const { data: { publicUrl } } = supabase.storage.from("avatars").getPublicUrl(`deliveries/${fileName}`);
